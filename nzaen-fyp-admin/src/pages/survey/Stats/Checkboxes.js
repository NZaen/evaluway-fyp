...
export default function Checkboxes({ survey }) {
    ...
    const ratingOptions = survey.formdata
        .filter((element) => getElementFieldValue(element.id) === 'Checkboxes')
        ...
        <div>
            ...
            {ratingOptions.map((option) => {
                const data = survey.assignedUsersList.reduce((acc, user) => {
                    const ratings = user.surveyAnswers && user.surveyAnswers[option] ? user.surveyAnswers[option] : [];
                ...
                return (
                    <div key={`chart-${option}`} className='statSec'>
                        <h2>{String(getElementFieldQuestionEn(option))}<span className='type'>Checkboxes</span></h2>
                        ...
                        <BarChart width={600} height={300} data={ratingFrequencyData}>
                            <XAxis dataKey="rating" label={{ value: 'Options', position: 'insideBottom', offset: -10 }} />
