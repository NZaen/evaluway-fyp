...
export default function RadioButtons({ survey }) {
    ...
    const ratingOptions = survey.formdata
        .filter((element) => getElementFieldValue(element.id) === 'RadioButtons')
        ...
        <div>
            {ratingOptions.map((option) => {
                const data = survey.assignedUsersList.reduce((acc, user) => {
                    const rating = user.surveyAnswers && user.surveyAnswers[option];
                    if (rating) {
                ...
                return (
                    ...
                    <div key={`chart-${option}`} className='statSec'>
                        <h2>{String(getElementFieldQuestionEn(option))}<span className='type'>RadioButtons</span></h2>
