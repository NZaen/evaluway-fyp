...
export default function Tags({ survey }) {
    ...
    const ratingOptions = survey.formdata
        .filter((element) => getElementFieldValue(element.id) === 'Tags')
            ...
            {ratingOptions.map((option) => {
                ...
                return (
                    <div key={`chart-${option}`} className='statSec'>
                        <h2>{String(getElementFieldQuestionEn(option))}<span className='type'>Tags</span></h2>
